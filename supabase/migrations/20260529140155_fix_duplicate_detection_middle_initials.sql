/*
  # Fix duplicate detection to handle middle initials in first names

  1. Changes
    - Updates `flag_duplicate_submissions()` to compare only the first word of `client_first_name`
      so "Dale" matches "Dale M", "Angela" matches "Angela D", etc.
    - Updates `flag_superseded_by_data_source()` with the same first-word comparison
    - This fixes cases where Data Source records include middle initials but
      Intake Form submissions only have the first name

  2. Important Notes
    - Uses `split_part(client_first_name, ' ', 1)` to extract just the first word
    - Preserves all existing logic (14-day window, status checks, Data Source priority)
    - Does not modify stored data, only comparison logic
*/
...
BEGIN
  ...
  WITH duplicate_groups AS (
    SELECT
      id,
      agent_number,
      LOWER(split_part(client_first_name, ' ', 1)) as fn,
      ...
      created_at,
      ROW_NUMBER() OVER (
        PARTITION BY agent_number, LOWER(split_part(client_first_name, ' ', 1)), LOWER(client_last_name), zip
        ORDER BY
...
BEGIN
  ...
  to_supersede AS (
    ...
    INNER JOIN data_source_records ds
      ...
      AND LOWER(split_part(fs.client_first_name, ' ', 1)) = ds.fn
