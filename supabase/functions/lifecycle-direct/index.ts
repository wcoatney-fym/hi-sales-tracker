...
// ── Akamai CA cert REMOVED (2026-09-15) ──────────────────────────────────────
// Max's Akamai/Postgres DB is dead (credentials rotated 2026-09-10).
// PROD_DB_* secrets now point at fym-production-db Supabase pooler.
// Standard TLS (ssl: "require") replaces the pinned Akamai CA cert.
...
Deno.serve(async (req: Request) => {
  ...
  // Legacy service_role/anon keys disabled 2026-06-16. Use publishable key for
  // Supabase client (reads + RLS-scoped writes like lifecycle_confirm_tokens).
  ...
  // Legacy vault RPC approach removed: service_role key disabled 2026-06-16.
  const expectedCronSecret = Deno.env.get("LIFECYCLE_CRON_SECRET") ??
                              Deno.env.get("ACTIVITY_TRACKER_SECRET_KEY") ?? "";
  if (cronSecret) {
