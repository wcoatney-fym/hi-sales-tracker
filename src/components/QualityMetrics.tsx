  ...
  return (
    <div className="bg-navy rounded-xl border border-slate-700/50 p-5 mt-6">
      ...
      <p className="text-xs text-slate-400 mb-4">
        Placement: submitted apps that drafted their first premium. Persistency: of policies that went
        active in a month, the share still active today.
      </p>
            ...
            {placement.map((p) => (
              <div key={p.month} className="rounded-lg bg-slate-800/50 border border-slate-700/50 p-3 text-center">
                ...
                <div className="text-[11px] text-slate-400">
                  {monthLabel(p.month)} · {p.placed}/{p.submitted}
                </div>
              </div>
            ))}
          </div>
